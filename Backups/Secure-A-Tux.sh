...
#IF THE SCRIPT FAILS AFTER USING ON WINDOWS, USE dos2unix CONVERSION
...
echo "INSERT PICTURE OF TUX HERE"


echo -e "\e[1;32mWelcome to Secure-A-Tux\e[0m"
...
echo -e "\e[0;36mInstalling gnome-system-tools for user and group management (Ensure proper UID and GIDs, with no duplicate or unbalanced permissions\e[0m"
...
read -rsp $'\e[1;31mManage UAC and then press space to continue...\e[0m\n' -n1 key
...
read -p "Do you want to import an /etc/anacrontab file? " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    mv /etc/anacrontab /etc/anacrontab.bkup
...
#Anacrontab and crontab
read -p "Do you want to import an /etc/crontab file? " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    mv /etc/crontab /etc/crontab.bkup
...
echo -e "\e[0;36mRemoving john the ripper (in case it is present)\e[0m"
apt-get -y remove john

read -rsp $'\e[1;31mEnsure no other accounts have a UID of zero besides root. If so, change the UID via gnome-system-tools. Press space to continue...\e[0m\n' -n1 key
awk -F: '($3 == "0") {print}' /etc/passwd

echo -e "\e[0;36mNow enabling UFW firewall\e[0m"
ufw enable
ufw logging high
#hosts.allow and hosts.deny
read -p "Do you want to import hosts.allow and hosts.deny? " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
...
apt-get -y install clamav clamav-daemon clamav-freshclam clamtk rkhunter chkrootkit 

echo -e "\e[0;36mNow running rkhunter. In the meantime, use clamtk to scan for viruses\e[0m"
rkhunter -c
...
#Is there an SSH server? If so, set PermitRootLogin to no and check x11 forwarding. Also, allow ssh through firewall
...
then
    ...
    cp /home/$username/SecureATux/sshd_config /etc/sshd_config
...
read -p "Are you on Ubuntu 14? [y/n] " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
...
#Ubuntu 16
read -p "Are you on Ubuntu 16? [y/n] " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
...
#Debian
read -p "Are you on Debian? [y/n] " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    sed '$ a allow-guest=false' /etc/lightdm/lightdm.conf
...
echo -e "\e[0;36mSetting minimum password requirements\e[0m"
apt-get -y install libpam-cracklib
read -rsp $'\e[0;36mConfigure common-password and common-auth manually [SPACE]\e[0m\n' -n1 key
read -rsp $'\e[0;36mEdit /etc/login_defs with same info as file in SecureATux directory [SPACE]\e[0m\n' -n1 key
mv /etc/login.defs /etc/login_defs.bkup
cp /home/$username/SecureATux/login.defs /etc/
useradd -D -f 30
#add Password inactivity variable to login.defs

#Setting permissions for /etc/passwd, /etc/shadow/, etc.
...
mv /etc/lightdm/users.conf /etc/lightdm/users_conf.bkup
...
if [[ $REPLY =~ ^[Yy]$ ]]
then
    mv /etc/apt/sources.list /etc/apt/sources_list.bkup
    mv /etc/apt/trustdb.gpg /etc/apt/trustdb_gpg.bkup
    mv /etc/apt/trusted.gpg /etc/apt/trusted_gpg.bkup
    ...
    #apt.conf.f
    mv /etc/apt/apt.conf.d/00aptitude /etc/apt/apt.conf.d/00aptitude.bkup
    mv /etc/apt/apt.conf.d/00trustcdrom /etc/apt/apt.conf.d/00trustcdrom.bkup
    mv /etc/apt/apt.conf.d/01autoremove /etc/apt/apt.conf.d/01autoremove.bkup
    mv /etc/apt/apt.conf.d/01autoremove-kernels /etc/apt/apt.conf.d/01autoremove-kernels.bkup
    mv /etc/apt/apt.conf.d/10periodic /etc/apt/apt.conf.d/10periodic.bkup
    mv /etc/apt/apt.conf.d/15update-stamp /etc/apt/apt.conf.d/15update-stamp.bkup
    mv /etc/apt/apt.conf.d/20archive /etc/apt/apt.conf.d/20archive.bkup
    mv /etc/apt/apt.conf.d/20changelog /etc/apt/apt.conf.d/20changelog.bkup
    mv /etc/apt/apt.conf.d/20dbus /etc/apt/apt.conf.d/20dbus.bkup
    mv /etc/apt/apt.conf.d/50unattended-upgrades /etc/apt/apt.conf.d/50unattended-upgrades.bkup
    mv /etc/apt/apt.conf.d/70debconf /etc/apt/apt.conf.d/70debconf.bkup
    mv /etc/apt/apt.conf.d/99update-notifier /etc/apt/apt.conf.d/99update-notifier.bkup
...
#CIS Cybersecurity extras
#Ubuntu 14
echo -e "\e[0;36mNow performing CIS Cybersecurity inspired hardening\e[0m"
read -p "Are you on Ubuntu 14.04? " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    ...
    echo -e "\e[1;32m----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\e[0m"
    #Configuring permissions for hosts.allow / hosts.deny
...
read -p "Are you on Ubuntu 16? " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    #Apparmor install
...
echo -e "\e[0;36mNow running updates\e[0m"
apt-get -y update
apt-get -y upgrade
